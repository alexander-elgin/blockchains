...
export default class TronScanBalanceExplorerResponseParser extends TxListExplorerResponseParser {
    parseList(res: TronGridTxListResponse) {
        return res.data.filter((txData) => txData.raw_data.contract
            .find((params) => params.type === 'TransferContract'));
    }

    parseItem(tx: TronGridTxRecord) {
        const receipt = tx.raw_data.contract.reduce((res, params) =>
            Object.assign({}, res, params.parameter.value), { amount: -1, owner_address: '', to_address: '' });
